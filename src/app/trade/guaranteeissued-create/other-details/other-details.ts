...
export class OtherDetails {
  ...
  constructor(private fb: FormBuilder) {
    this.form = this.fb.group({
      ...
      periodicChargeCycleEndDateAdjustment: ['', Validators.required]
    });
  }
  ...
  cycleOptions = [
    ...
    'Both dates on next working day (keep same month)',
    'Both dates on previous working day',
    'Both dates on previous working day (keep same month)',
    'Due date only on next working day',
    'Due date only on next working day (keep same month)',
    'Due date only on previous working day (keep same month)'
  ];
}